...
class pgOutputDataBundle(val IN_WIDTH:Int, val C_WIDTH:Int) extends Bundle{
    val pp_out  = Output(Vec(IN_WIDTH/2, UInt(C_WIDTH.W)))
    val sig_out = Output(UInt(C_WIDTH.W))
}
...
class ppGenerator(val IN_WIDTH: Int, val C_WIDTH: Int) extends Module {
    ...
    val pp         = RegInit(VecInit(Seq.fill(IN_WIDTH/2)(0.U(C_WIDTH.W))))
    ...
    for (i <- 0 until IN_WIDTH/2){
        ...
        if (i == 0){
            if(C_WIDTH-IN_WIDTH-4 == 0){
                pp_temp(i) := Cat(E(0), E_inv(0).asUInt, E_inv(0).asUInt, pp_inv(0))
            } else {
    ...
    for (i <- 0 until IN_WIDTH/2){
        if(i == 0){
            ...
            pp(1) := Cat(pp_temp(i)(C_WIDTH-2*i-1,0), 0.U(1.W), inputCtrl.inv(i-1))
        } else {
            pp(i) := Cat(pp_temp(i)(C_WIDTH-2*i-1,0), 0.U(1.W), inputCtrl.inv(i-1), 0.U((2*(i-1)).W))
        }
    }
    sign_com := Cat(0.U((C_WIDTH-7).W), inputCtrl.inv(IN_WIDTH/2-1), 0.U((2*(IN_WIDTH/2-1)).W))
